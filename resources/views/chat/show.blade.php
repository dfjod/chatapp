<x-app-layout>
        <x-slot name="currentChat">
            <x-nav-link :href="route('chats.edit', $currentChat)" :active="request()->routeIs('chats.edit')">
                {{ __($currentChat->name) }}
            </x-nav-link>
        </x-slot>
        <div class="overflow-y-auto px-10 py-5 w-full">
            @foreach ($messages as $message)
                <div class="shrink-1 flex flex-col w-full max-w-[320px] leading-1.5 p-4 border-gray-200 {{ auth()->user()->id === $message->user->id ? 'bg-blue-100 ml-auto' : 'bg-gray-200' }} rounded-xl mb-5 mt-5 first:mt-0 last:mb-0">
        ...
        <div class="px-10 pb-5 pt-0 w-full">
            <form method="POST" action="{{ route('messages.send', $chat) }}">
                @csrf
    
                <label for="content" class="sr-only">Your message</label>
                <div class="flex items-center py-2 rounded-lg">
                    <textarea id="content" name="content" rows="1" class="block p-2.5 w-full text-sm text-gray-900 bg-white rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500" placeholder="Your message..."></textarea>
